...
////  Specifications :  //////////////////////////////////////////////////////////////////////////////////////////////////////
                                                                                                                            //
//    1.Solar panel power = 528W                                            
                                                                                                                            //
//    2.Rated Battery Voltage= 14.7V x 3 ( lead acid type ) #(2/25/17) changed to reflect battery bank voltage

//    3.Maximum current = 12.5-13.0A                               #(2/25/17) changed to our max current in PRD                                                                 //

//    4.Maximum load current = 18A                                                                                            //

//    5.Input Voltage = Solar panel with Open circuit voltage from 8 to 52V                                               //
...
#include <SPI.h>          // This file is used for the Digital-Analog Converter
//----------------------------------------------------------------------------------------------------------
...
#define BATT_FLOAT 38.52            // battery voltage we want to stop charging at   //(2/25/17) changed for our batteries
#define BATT_ABSORB 44.10           //battery voltage we want to be in absorption at
#define HIGH_BAT_VOLTS 39.00        //value of battery voltage // this is 13.00 volts 
#define LVD 34.5                    //Low voltage disconnect setting for a 12V system
...
int pwm_dac = 4*pwm;                      // value of pwm converted for use in the D-A-C
  
enum charger_mode {off, on, bulk, bat_float, bat_absorb} charger_state;    // enumerated variable that holds state for charger state machine
...
//------------------------------------------------------------------------------------------------------
void set_pwm_duty(void) {            //setting pwm (pulse width modualtor) 
...
//
//  Absorption State - Once the battery charges to about 75-80% of full capacity, we want to keep the voltage constant
//      and let the current slowly decrease. This is mainly to prevent the battery from being overcharged or undercharged.
//
//  Float State - As the battery charges its voltage rises. When it gets to the MAX_BAT_VOLTS we are done with the 
...
void run_charger(void) {
  ...
  switch (charger_state) {
    case on:                                        
      ...
      else if (bat_volts > (BATT_ABSORB - 0.1)) {            // else if the battery voltage has gotten above the absorption
        charger_state = bat_absorb;                          // battery voltage go to the charger battery absorption state
      }
    ...
    case bulk:
      ...
      else if (bat_volts > BATT_ABSORB) {                    // else if the battery voltage has gotten above the float
        charger_state = bat_absorb;                          // battery float voltage go to the charger battery float state
      }
      else if (sol_watts < LOW_SOL_WATTS) {                 // else if the solar input watts is less than low solar watts
     ...
     case bat_absorb:
      ...
      else if (bat_volts > BATT_ABSORB) {                    // If we've charged the battery above the float voltage                
        TURN_OFF_MOSFETS;                                   // turn off MOSFETs instead of modifying duty cycle
        pwm = PWM_MAX;                                      // the charger is less efficient at 99% duty cycle
        set_pwm_duty();                                     // write the PWM
      }
      else if (bat_volts < BATT_ABSORB) {                    // else if the battery voltage is less than the absorption voltage - 0.1
        pwm = PWM_MAX;                                      
        set_pwm_duty();                                     // start charging again
        TURN_ON_MOSFETS;    
        if (bat_volts < (BATT_ABSORB - 0.1)) {               // if the voltage drops because of added load,
    ...
    case bat_float:
      ...
      else if (bat_volts > BATT_FLOAT) {                    // If we've charged the battery above the float voltage                
...
void print_data(void) {
  ...
  else if (charger_state == bat_absorb) Serial.print("absorption");
...
void lcd_display()
{
 ...
 else if (charger_state == bat_absorb)
 {
  lcd.print("     ");
  lcd.setCursor(8,2);
 ...
 //-----------------------------------------------------------
 lcd.setCursor(8,3);
 if ( bat_volts >= 38.52)                               //(2/25/17) changed to reflect our batteries' voltages
 lcd.print( "100%");
 //else if (bat_volts >= 12.5 && bat_volts < 12.7)
 //lcd.print( "90%");
 //else if (bat_volts >= 12.42 && bat_volts < 12.5)
 //lcd.print( "80%");
 else if (bat_volts >= 37.5 && bat_volts < 38.52)       //(2/25/17) changed to reflect our batteries' voltages
 lcd.print( "75%");
 //else if (bat_volts >= 12.2 && bat_volts < 12.32)
 //lcd.print( "60%");
 else if (bat_volts >= 36.54 && bat_volts < 37.5)       //(2/25/17) changed to reflect our batteries' voltages
 lcd.print( "50%");
 //else if (bat_volts >= 11.90 && bat_volts < 12.06)
 //lcd.print( "40%");
 //else if (bat_volts >= 11.75 && bat_volts < 11.90)
 //lcd.print( "30%");
 else if (bat_volts >= 35.64 && bat_volts < 36.54)      //(2/25/17) changed to reflect our batteries' voltages
 lcd.print( "25%");
 //else if (bat_volts >= 11.31 && bat_volts < 11.58)
 //lcd.print( "10%");
 else if (bat_volts < 35.1)                            //(3/09/17) battery at less than 10% charge, we consider it to be 0%
 lcd.print( "0%");
